...
import { anno, checkRange, clienteNameResolver, dateSchema, defineTool, euro, isIncassata, limitSchema, offsetSchema, round2, snapshotOf, sortByData, userIdSchema, withClienteNome } from './shared';

export const listFatture = defineTool({
  name: 'list_fatture',
  title: 'Fatture emesse',
  description: 'Fatture del profilo per data di emissione, con totali su tutto il filtro: importo (emesso), incassato e daIncassare (la differenza). Risponde a "quanto ho emesso e quanto manca all\'appello". Per "quanto ho incassato nell\'anno" usa get_riepilogo_anno, che va per cassa.',
  input: {
    userId: userIdSchema,
  ...
  async handler(ctx, { userId, anno: year, incassata, clienteId, da, a, limit, offset }) {
    ...
    const { items, total, hasMore } = paginate(withClienteNome(filtered, clienteNameResolver(snap.clienti)), limit, offset);
    return {
