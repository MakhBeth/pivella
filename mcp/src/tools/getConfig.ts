...
export const getConfig = defineTool({
  ...
  async handler(ctx, { userId }) {
    ...
    const text = config
      ? `Config di ${snap.user.nome}: P.IVA ${config.partitaIva || 'non impostata'}, apertura ${config.annoApertura}, ${config.gestionePrevidenziale}, emittente ${structured.emittenteConfigurato ? 'configurato' : 'non configurato'}`
